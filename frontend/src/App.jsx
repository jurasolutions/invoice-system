...
import { Alert, Button, Icon, Logo, Toast } from "./ds.js";
...
function Shell() {
  ...
  const { status, error, data, toasts, session, signIn } = useApp();

  if (status === "loading") {
    return <Splash>Loading…</Splash>;
  }

  // Nothing behind this point renders without a session. The API enforces the
  ...
  return (
    <div className="app">
      <AppBar route={route} />
      <DefaultCredentialsWarning session={session} />
...
function AppBar({ route }) {
  const { data, session, signOut } = useApp();
  const links = [
...
/**
 * The app is running on the password that is published in this repo.
 *
 * Harmless on a laptop, and the reason the server refuses to start in
 * production without a real one. Worth saying out loud either way, because the
 * gap between "it works" and "it is safe to reach from anywhere" is exactly
 * this.
 */
function DefaultCredentialsWarning({ session }) {
  if (!session?.using_default_credentials) return null;
  return (
    <div className="container" style={{ padding: "var(--space-4) var(--space-5) 0" }}>
      <Alert tone="warning" title="Running on the default password">
        This instance still accepts <code>admin</code> / <code>P@ssw0rd</code>, which is published in the repo. Fine
        locally. Before this is reachable from anywhere else, run{" "}
        <code>npm run hash-password -- 'your password'</code> and set <code>ADMIN_PASSWORD_HASH</code>.
      </Alert>
    </div>
  );
}
