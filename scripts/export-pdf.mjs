...
import { paths, repoRoot } from "../server/paths.mjs";
import { listDocuments } from "../server/store.mjs";
...
try {
  for (const summary of wanted) {
    ...
    const target = `${url.replace(/\/$/, "")}/#/print/${encodeURIComponent(summary.number ?? summary.id)}`;
